...
def map_overall_state_to_message_dto(overall_state: OverallState) -> MessageSummaryDTO:
    ...
    # Use the LAST message (current turn), not messages[0] (first in session).
    # In multi-turn sessions, messages[0] is the original opening query; the
    # current request is always the last human message in the list.
    human_query = overall_state.messages[-1].text if overall_state.messages else None
